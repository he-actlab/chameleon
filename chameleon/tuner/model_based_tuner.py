...
class ModelBasedTuner(Tuner):
    ...
    def __init__(self, task, cost_model, model_optimizer, plan_size, diversity_filter_ratio=None, sampler=None):
        ...
        self.trial_pt = 0
        self.visited = set()
        # for every run of optimizer, use plan_size as a start.
        # with sampler, this is to be adjusted dynamically
    ...
    def update(self, inputs, results):
        ...
        if len(self.visited) >= self.next_update \
            ...
            self.trial_pt = 0
            self.train_ct += 1
            # add the number of samples from the optimizer/sampler to the next_update
